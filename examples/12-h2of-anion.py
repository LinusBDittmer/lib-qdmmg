...
s = sim.Simulation(1000, 1.0, dim=6, verbose=9, generations=3)
geometry, charge, mult = pot.from_xyz('./benchmarking/H2OF-/geometry/base.xyz')
p = pot.MolecularPotential(s, geometry, charge=charge, multiplicity=mult, rounding=5, basis='6-31g', theory='rhf')
s.bind_potential(p)
s.gen_wavefunction()
exp.export_to_json(s.get_wavefunction(), './benchmarking/H2OF-/wavefunction.json')
...
plot.density_plots(s.get_wavefunction(), name='./benchmarking/H2OF-/h2of_amplitude', drawtype='amplitude')
plot.linear_plots(autocorrelation, fauto, populations, name='./benchmarking/H2OF-/h2of_properties')
