...
RETURN_VALUE=0
NODE_WORKDIR="/scratch/ccprak16/base_double_anion_$SLURM_JOB_ID"
NODE_SCRATCHDIR="/lscratch/ccprak16/base_double_anion_$SLURM_JOB_ID"
#
###################################
#
 print_info() {
...
payload_hooks() {
:
for FILEORDIRPATH in $SLURM_SUBMIT_DIR/base_double_anion.in; do
...
/export/home/ccprak16/dreuwBin/qchem/versions/qchem-6.0 -nt 4 "base_double_anion.in" "base_double_anion.out"
RETURN_VALUE=$?

# check if job terminated successfully
if ! tail -n 30 "base_double_anion.out" | grep -q "Thank you very much for using Q-Chem.  Have a nice day."; then
    RETURN_VALUE=1
fi

for FILEORDIRPATH in $NODE_WORKDIR/base_double_anion.out; do
...
for FILEORDIRPATH in $NODE_WORKDIR/base_double_anion.in.fchk; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
...
for FILEORDIRPATH in $NODE_WORKDIR/base_double_anion.out.plots; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
